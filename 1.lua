--[[
    ADITYA_ORG MOD - Fixed Self-Injecting Edition
    All features now work correctly (iPad view, magic bullet, auto aim, weapon mods, no recoil, no grass, black sky)
]]

-- Per-match guard: reset when player pawn changes (new match)
do
...
-- ==================== FEATURE IMPLEMENTATIONS (improved) ====================
...
-- ==================== MOD MENU (same as original, works) ====================
...
-- ==================== MAIN TICK (same as OnTick in original) ====================
local function AdityaMainTick()
    pcall(function()
        ...
        -- Always keep grass and sky settings applied (they are toggles)
        _G.DisableGrass()
        _G.BlackSky()
        ...
        -- Auto aim (runs every tick as in original)
        _G.ApplyAutoAim()
    end)
end

-- ==================== AUTO-START WITH PROPER INITIALIZATION ====================
local function StartAdityaTimer()
    local pc = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
    if not (pc and slua.isValid(pc)) then
        if slua_GameFrontendHUD and slua_GameFrontendHUD.AddGameTimer then
            slua_GameFrontendHUD:AddGameTimer(1.0, false, StartAdityaTimer)
        end
        return
    end
    if _G._ADITYA_TIMER then
        -- Already running, but ensure it's on the correct PC
        if _G._ADITYA_PC ~= pc then
            if _G._ADITYA_PC and slua.isValid(_G._ADITYA_PC) then
                pcall(function() _G._ADITYA_PC:RemoveGameTimer(_G._ADITYA_TIMER) end)
            end
            _G._ADITYA_TIMER = nil
        end
    end
    if not _G._ADITYA_TIMER then
        _G._ADITYA_TIMER = pc:AddGameTimer(0.2, true, AdityaMainTick)
        _G._ADITYA_PC = pc
    end
end

-- Run bypasses and menu initialization once
pcall(function()
    _G.TryBypassMD5()
    _G.BypassCacheMD5()
