...
pcall(function()
    ...
    if Msg and Msg.Show then
        Msg.Show(4, "@ADITYA_ORG", 
            "╔══════════════════════════════════╗\n" ..
            "║     WELCOME TO ADITYA OFFICIAL MOD        ║\n" ..
            "╠══════════════════════════════════╣\n" ..
            "║ ✓ COMPLETE BYPASS ACTIVE         ║\n" ..
            "║ ✓ SERVER BASE UPDATE AUTO      ║\n" ..
...
local function ESPTick()
    ...
    if not crowded and HUD and currentPawn then
        HUD:AddDebugText(string.format("BOT : %d     PLAYER : %d", botCount, playerCount), currentPawn, 1, {X=0,Y=0,Z=160}, {X=0,Y=0,Z=160}, {R=255,G=255,B=0,A=255}, true, false, true, nil, 1.0, true)
        HUD:AddDebugText("MODDED BY @ADITYA_ORG", currentPawn, 1, {X=0,Y=0,Z=145}, {X=0,Y=0,Z=145}, {R=0,G=255,B=255,A=255}, true, false, true, nil, 1.0, true)
    end
end
            ...
            if isValid(mesh) then
              ...
              if isValid(physAsset) and physAsset.SkeletalBodySetups then
                ...
                if not _G._MBones[assetName] then
                  local mb = {
                    ["head"]=0, ["neck_01"]=60, ["pelvis"]=60,
                    ["spine_01"]=60, ["spine_02"]=60, ["spine_03"]=60,
                    ["upperarm_l"]=60, ["upperarm_r"]=60,
                    ["lowerarm_l"]=60, ["lowerarm_r"]=60,
                    ["hand_l"]=60, ["hand_r"]=60,
                    ["thigh_l"]=60, ["thigh_r"]=60,
                    ["calf_l"]=60, ["calf_r"]=60,
                    ["foot_l"]=60, ["foot_r"]=60,
                  }
...
local function ApplyHardAimbot()
    ...
    pcall(function()
        ...
        entity.GameDeviationFactor = 0.9
